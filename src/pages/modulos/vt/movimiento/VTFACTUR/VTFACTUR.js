...
const FormName     = 'VTFACTUR';
const TituloList   = "Facturas";
...
const MaiST = memo(() => {
  ...
  const calculaTotales = async() => {
    ...
    lineas.map( item => {
      if( item.PORC_IVA === 5 ){
        tot_iva += Math.round( item.MONTO_TOTAL_C_IVA  / ( 1 + ( item.PORC_IVA / 100 ) ) * ( item.PORC_IVA / 100 ), 0 );
        grav5 += Math.round( item.MONTO_TOTAL_C_IVA - ( item.MONTO_TOTAL_C_IVA  / ( 1 + ( item.PORC_IVA / 100 ) ) * ( item.PORC_IVA / 100 ) ), 0 );
      }else if( item.PORC_IVA === 10 ){
        tot_iva += Math.round( item.MONTO_TOTAL_C_IVA  / ( 1 + ( item.PORC_IVA / 100 ) ) * ( item.PORC_IVA / 100 ), 0);
        grav10 += Math.round( item.MONTO_TOTAL_C_IVA - ( item.MONTO_TOTAL_C_IVA  / ( 1 + ( item.PORC_IVA / 100 ) ) * ( item.PORC_IVA / 100 ) ), 0 );
      }else if( item.PORC_IVA === 0 ){
        tot_exentas += item.MONTO_TOTAL_C_IVA;
      }
